...
public class EtudiantCoursDao implements IGestion<EtudiantCours> {

	@Override
	public boolean ajouter(EtudiantCours pEtudiantCours) {
	...
	public boolean supprimer(int id) {
		...
		EtudiantCours EtudiantCoursSupp = em.find(EtudiantCours.class, id);

		// 4.3 suppr du formateur dans la Bdd
		em.remove(EtudiantCoursSupp);
	...
	public EtudiantCours getById(int id) {
		 ...
		 */
		EtudiantCours EtudiantCours = em.find(EtudiantCours.class, id);
	...
	public void modifier(EtudiantCours EtudiantCours) {
		...
		EtudiantCours EtudiantCoursModif = em.find(EtudiantCours.class, EtudiantCours.getIdEtudiantCours());

		// donc on modifie l'objet etudiantEtudiantCoursMotif avec les paramètres de etudiantEtudiantCours
		EtudiantCoursModif.setIdEtudiantCours(EtudiantCours.getIdEtudiantCours());
		EtudiantCoursModif.setMotif(EtudiantCours.getMotif());
		EtudiantCoursModif.setAbsence(EtudiantCours.getAbsence());
		EtudiantCoursModif.setEtudiant(EtudiantCours.getEtudiant());
		EtudiantCoursModif.setCours(EtudiantCours.getCours());



		// 4.3 modif du formateur dans la Bdd
		EtudiantCours EtudiantCoursMerged = em.merge(EtudiantCoursModif);

		// // 4.4 affichage de la modif
		// System.out.println(etudiantEtudiantCoursMerged);
	...
	public List<EtudiantCours> getAll() {
		...
		// 2.2.1 Construction du FROM de la requête : FROM etudiantEtudiantCours e
		Root<EtudiantCours> clauseFrom = criteriaQuery.from(EtudiantCours.class);

		// 2.2.2 Construction du SELECT de la requête : SELECT e
		CriteriaQuery<EtudiantCours> clauseSELECT = criteriaQuery.select(clauseFrom); // => JPSL : SELECT e FROM etudiant e

		// 3. Transmission de la requête Criteria à l'EntityManager
		TypedQuery<EtudiantCours> getAllQuery = em.createQuery(clauseSELECT);

		// 4. Exécution et récup du résultat de la requête
		List<EtudiantCours> listeEtudiantCourss = getAllQuery.getResultList();
