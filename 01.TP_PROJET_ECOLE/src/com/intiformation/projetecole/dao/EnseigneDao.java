...
import com.intiformation.projetecole.entity.Enseigne;
import com.intiformation.projetecole.entity.Enseigne;

public class EnseigneDao implements IGestion<Enseigne> {

	@Override
	public boolean ajouter(Enseigne pEnseigne) {
		 ...
		 */
		// try catch (rollback)
		entityManager.persist(pEnseigne);
	...
	public boolean supprimer(int id) {
		...
		Enseigne enseigneSupp = em.find(Enseigne.class, id);

		// 4.3 suppr du formateur dans la Bdd
		em.remove(enseigneSupp);
	...
	public Enseigne getById(int id) {
		 ...
		 */
		Enseigne enseigne = em.find(Enseigne.class, id);
	...
	@Override
	public void modifier(Enseigne enseigne) {
		EntityManagerFactory emf = Persistence.createEntityManagerFactory("01.TP_PROJET_ECOLE");
		...
		Enseigne enseigneModif = em.find(Enseigne.class, enseigne.getIdEnseigne());

		// donc on modifie l'objet enseigneMotif avec les paramètres de enseigne
		enseigneModif.setIdEnseigne(enseigne.getIdEnseigne());
		enseigneModif.setEnseignant(enseigne.getEnseignant());
		enseigneModif.setMatiere(enseigne.getMatiere());
		enseigneModif.setPromotion(enseigne.getPromotion());
		


		// 4.3 modif du formateur dans la Bdd
		Enseigne enseigneMerged = em.merge(enseigneModif);

		// // 4.4 affichage de la modif
		// System.out.println(enseigneMerged);
	...
	public List<Enseigne> getAll() {
		...
		CriteriaQuery<Enseigne> criteriaQuery = criteriaBuilder.createQuery(Enseigne.class);

		/**
		 * Reproduction de la requête JPQL : SELECT e FROM etudiant e
		 */

		// 2.2.1 Construction du FROM de la requête : FROM enseigne e
		Root<Enseigne> clauseFrom = criteriaQuery.from(Enseigne.class);

		// 2.2.2 Construction du SELECT de la requête : SELECT e
		CriteriaQuery<Enseigne> clauseSELECT = criteriaQuery.select(clauseFrom); // => JPSL : SELECT e FROM etudiant e

		// 3. Transmission de la requête Criteria à l'EntityManager
		TypedQuery<Enseigne> getAllQuery = em.createQuery(clauseSELECT);

		// 4. Exécution et récup du résultat de la requête
		List<Enseigne> listeEnseignes = getAllQuery.getResultList();
